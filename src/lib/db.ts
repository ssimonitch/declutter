// Database & Storage Layer for SuzuMemo App
...
export class SuzuMemoDatabase extends Dexie {
  items!: Table<SuzuMemoItemDB>;
  ...
  constructor() {
    ...
    super("SuzuMemoDB", { addons: [dexieCloud] });
...
let dbInstance: SuzuMemoDatabase | null = null;
 ...
 */
export function getDb(): SuzuMemoDatabase {
  if (!dbInstance) {
    if (typeof window === "undefined") {
      throw new Error("SuzuMemoDatabase must be used in the browser");
    }
 ...
 */
export async function getPendingInvitations(): Promise<SuzuMemoMember[]> {
  try {
    const db = getDb();
    if (!db.cloud?.currentUser) {
      return [];
 ...
 */
export function canShareItem(item: SuzuMemoItem): boolean {
  return !item.realmId;
}
...
export async function addItem(
  item: Omit<SuzuMemoItem, "id" | "createdAt" | "updatedAt">,
  realmId?: string | null,
): Promise<string> {
  try {
    const now = new Date().toISOString();
    ...
    const newItem: Omit<SuzuMemoItem, "id"> = {
      ...item,
 ...
 */
export async function getRealm(realmId: string): Promise<SuzuMemoRealm | null> {
  try {
    const db = getDb();
    if (!db.cloud?.currentUser) {
      return null;
