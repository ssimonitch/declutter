...
export async function createRealm(
  ...
  try {
    ...
    const realmId = await db.transaction("rw", ["realms"], async () => {
      // Let Dexie Cloud auto-generate the realm ID with correct prefix
      // When using '@' in schema, the ID is auto-generated
      const newRealmId = await db.realms.add({
        // realmId will be auto-generated by Dexie Cloud
        name: request.name,
        owner: currentUser.userId,
      });

      // Note: The owner doesn't need to be added as a member
      // Dexie Cloud automatically grants the owner access to their realm
...
export async function inviteMember(
  ...
  try {
    ...
    // Note: When using '@' prefix in schema, Dexie Cloud auto-generates the ID with correct prefix
    const inviteId = await db.transaction("rw", "members", async () => {
      // Don't manually generate ID - let Dexie Cloud handle it
      const newInviteId = await db.members.add({
        // id is auto-generated when using '@' in schema
...
// Note: Global realm state has been removed.
// All database functions now require explicit realmId parameter.
// Use RealmContext via useRealm() hook in UI components to get current realm.
...
// ================================
// These database functions require an explicit `realmId` parameter for realm filtering.
//
...
//
// The realmId parameter is required to ensure consistent state management
// and avoid any drift between React context and database operations.
 ...
 * @param realmId - Required realm ID to filter by. Pass null for private items only.
 *                  Get this from useRealm() hook in UI components.
 */
export async function listItems(
  realmId: string | null,
): Promise<SuzuMemoItem[]> {
  try {
    const effectiveRealmId = realmId;
    const db = getDb();
...
export async function leaveRealm(realmId: string): Promise<void> {
  try {
    ...
    // Note: UI components should use RealmContext to manage realm state
    // Database functions no longer manage global realm state
